    ...
    private void SetUp()
    {
        for(int i=0; i < width; i++)
        {
            for (int j = 0; j < height; j++)
            {
                ...
                int maxIter = 0;
                while(MatchesAt(i,j,dots[dotChoosen]) && maxIter < 100){
                    dotChoosen = Random.Range(0, dots.Length);
                    maxIter++;
                }
                maxIter = 0;
        ...
        for(int i = 0; i < width; i++){
            for(int j = 0; j < height; j++){
                if(allDots[i,j] == null){
                    Vector2 tempPos = new Vector2(i,j + offset);
                    int dotChoosen = Random.Range(0, dots.Length);
                    GameObject piece = Instantiate(dots[dotChoosen],tempPos, Quaternion.identity);
                    allDots[i,j] = piece;
